...
server <- shiny::shinyServer(function(input, output){
  ...
  pred_quali_glm <- reactive({
    ...
    vaX <- foreach(va_quali = input$quali_X_glm) %do% {conversion(va_quali)}
    if (input$radio_X_add_glm == 1 && length(input$quanti_X_glm) != 0) {
      print(input$quanti_X_glm)
      vaX <- c(vaX, input$quanti_X_glm)
    }
    x <- data[, full(vaX, input$type_glm)]
    ...
    finaldata$Target <- ifelse(finaldata[, input$radio_Y_quali_glm] == input$target, 1, 0)
    finaldata$Target <- as.factor(finaldata$Target)
    
    if (!all(finaldata$Target == 0)) {
      y <- as.factor(finaldata$Target)
      ...
      obs_pred_data_quali <- data.frame(Observed = finaldata$Target, Predicted = glm_preds)
  ...
  output$prediction_plot <- renderPlot({
    ...
    if (input$model == "Linear model") {
      plot(x = obs_pred_data()$Predicted, y = obs_pred_data()$Observed,
      ...
      if (input$radio_Y_type_glm == 1) {
        ...
        if (!is.null(obs_pred_data_quali)) {
          plot(x = obs_pred_data_quali$Predicted, y = obs_pred_data_quali$Observed,
               xlab = "Predicted values (g.kg-1)", ylab = "Observed values (g.kg-1)",
               main = "Predicted VS. Observed values")
          abline(a = 0, b = 1, col = "red")
        }
